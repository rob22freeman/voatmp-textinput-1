...
Xrm.Portal = {
  ...
  Utility: {
      ...
      setValidation: function(groupObj, control, isRequired, validationGroup, validationFunction, customMessage) {
        ...
        if (c.length > 0) {
          ...
          if (Xrm.Portal.Utility.hasPage_Validators()) {
			...
			//<<HSL - changes
            nv.errormessage = "<a href='#" + l + "' onclick=\"javascript:scrollToAndFocus('" + l + "', '" + id + "');return false;\">" + m + "</a>";
			...
			//<<HSL - changes
			//append validator Element in DOM
			var vidSelector = Xrm.Portal.Utility.Selector.appendSelector(vid);
			var nvElementsFound = $(g).find(vidSelector);
			if(nvElementsFound && nvElementsFound.length === 0)
			{
				var validatorsElement = $(g).children(".validators").first();
				if(validatorsElement && validatorsElement.length > 0)
				{
					validatorsElement.append(nv);
				}else
				{
					$(g).append("<div class='validators'> " + nv + "</div>"); 
				}
			}
            
			// Sort Validation
			let validatorsElements = []; 
			$("#EntityFormView").find(".validators > span").map( (i, element) => validatorsElements.push(element.id));
			if(validatorsElements.length > 0 )
			{
				var indexOfnv = validatorsElements.indexOf(vid);
				if(indexOfnv !== -1)
				{
					Page_Validators.splice(indexOfnv, 0, nv);
				}else
				{
    ...
    //<<HSL - changes
	Accessibility : {
		setRequired: function(c, isRequired){
			var id = c.id;
			console.log("Accessibility.setRequired -> id: " + c);
			var control = document.getElementById(id);
			if (control) {
				//add accessibility tags
				 control.setAttribute("aria-required",isRequired);
				 control.removeEventListener("focus",  validateRequiredField(id));
				 control.addEventListener("focus",  validateRequiredField(id));
			}
		}
	} //HSL - changes>>
  },
  ...
  Form: {
    ...
    get: function(id) {
      ...
      switch (ct) {
        ...
        case this.controlType.Lookup:
          ...
          break;
        //<<HSL - changes
		case this.controlType.Money:
		  v = new Xrm.Portal.Control.Money(c);
          break;
        //HSL - changes>>
    ...
    getControlType: function(c) {
      ...
      if (c.length > 0) {
        ...
        //<<HSL - changes
		} else if (c.attr("class") != null && (c.attr("class").indexOf("money") >= 0)) {
          return this.controlType.Money;
         //HSL - changes>>
  ...
  Control: {
    ...
    Generic: function(c) {
      ...
      this.setRequired = function(isRequired, customFunction, customMessage) {
        ...
        Xrm.Portal.Utility.Accessibility.setRequired(this, isRequired); // HSL - changes
        return this;
      };
	...
	Money: function(c) {
      ...
      this.transformToCanvas = function() {
        this.c.hide();
        if (this.c.parent().parent().children().last()[0].tagName !== "CANVAS") {
          var canvasId = this.id + "Canvas";
          this.c.parent().parent().append("<canvas id=" + canvasId + "></canvas>");
          Xrm.Portal.Control.Canvas(this.id);
        }
      }
    },
